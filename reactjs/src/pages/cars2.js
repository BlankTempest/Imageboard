...
export class Car2a extends React.Component
{
    ...
    {
        super()
        this.state={disp:0,push:0,names:'',subjects:'',comment:'',image:'',temps:''}
    }
    
    componentDidMount(){
        // axios.get('http://localhost:8000/temps').then(res=>{
        //     console.log('here')
        //     // console.log(res.data)
        // })
        axios.post('http://localhost:8000/temps',{}).then((response)=>{console.log(response)})
    }
    render(){
        ...
        if(this.state.disp)
        {
            return(
                ...
                <div style={{"text-align":"center"}}>
                 <form onSubmit={this.push} method="post" action='/upload' id='uploadForm' encType="multipart/form-data">
    ...
    push= async (e)=>{
        ...
        const new_post=new Object({names,subjects,comment,image})
        // console.log(new_post)
        axios.post('/upload',new_post).then((response)=>{console.log(response.data)})
